...
namespace EMo.Core
{
    ...
    public class EventSubscriber
    {
        ...
        {
            var returnType = GetDelegateReturnType(eventSignature);
            var parameters = GetDelegateParameterTypes(eventSignature);
            ...
            var methodToCall = typeof(IEventRecorder).GetMethod("RecordEvent", BindingFlags.Instance | BindingFlags.Public);

            var ilGen = eventHandler.GetILGenerator();
            ...
            for(var index=0; index < parameters.Length; index++)
            {
                ...
                if( parameters[index].IsValueType ) ilGen.Emit(OpCodes.Box, parameters[index]);
